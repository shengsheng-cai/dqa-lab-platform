...
export default function DatePicker({ value, onChange, style, label = "" }) {
  ...
  return (
    <div style={{ display: "flex", gap: 3, alignItems: "center" }}>
      <select aria-label={label ? `${label}：年` : "年"} value={year}  onChange={(e) => emit(e.target.value, month, day)} style={{ ...sel, width: 64 }}>
        {years.map((y) => <option key={y} value={y}>{y}</option>)}
      </select>
      <span style={lbl} aria-hidden="true">年</span>
      <select aria-label={label ? `${label}：月` : "月"} value={month} onChange={(e) => emit(year, e.target.value, day)} style={{ ...sel, width: 44 }}>
        {months.map((mo) => <option key={mo} value={mo}>{String(mo).padStart(2, "0")}</option>)}
      </select>
      <span style={lbl} aria-hidden="true">月</span>
      <select aria-label={label ? `${label}：日` : "日"} value={day}   onChange={(e) => emit(year, month, e.target.value)} style={{ ...sel, width: 44 }}>
