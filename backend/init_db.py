...
def _dt(**kw) -> datetime.datetime:
    """回傳 UTC 現在時間減去 timedelta。正數 = 過去，負數 = 未來。"""
...
with SessionLocal() as db:
    ...
    sch1 = Schedule(
        ...
        sample_name="IEC60068 熱循環模組測試",
        applicant_name="陳工",
        device_id="CH-01",
        standard="IEC 60068",
        conditions=json.dumps(["iec60068_nb_-40_+85_5cycle"]),
        start_time=_dt(hours=2),
        end_time=_dt(hours=-10),
        status="進行中",
        current_condition_index=0,
    )
    sch2 = Schedule(
        project_number="PRJ-2025-091",
        sample_name="EN50155 軌道車輛環境測試",
        applicant_name="林工",
        device_id="CH-02",
        standard="EN 50155",
        conditions=json.dumps(["iec60068_nb_-25_+70_3cycle"]),
        start_time=_dt(hours=2, minutes=50),
        end_time=_dt(hours=-7),
        status="進行中",
        current_condition_index=0,
    )
    sch3 = Schedule(
        project_number="PRJ-2025-095",
        sample_name="IEC61850 繼電保護裝置高溫測試",
        applicant_name="王工",
        device_id="CH-04",
        standard="IEC 61850-3",
        conditions=json.dumps(["iec61850_ed2_c1_high"]),
        start_time=_dt(hours=-20),
        end_time=_dt(hours=-36),
        status="已確認",
        current_condition_index=0,
    )
    sch4 = Schedule(
        project_number="PRJ-2025-099",
        sample_name="IEC60068 低溫儲存評估",
        applicant_name="張工",
        device_id=None,
        standard="IEC 60068",
    ...
    sch5 = Schedule(
        project_number="PRJ-2025-072",
        sample_name="EN50155 完成品高低溫驗收",
        applicant_name="陳工",
        device_id="CH-04",
        standard="EN 50155",
        conditions=json.dumps(["en50155_2017_ot3_high", "en50155_2017_ot3_low"]),
        start_time=_dt(days=7),
        end_time=_dt(days=6),
        status="已完成",
        current_condition_index=1,
    )
    sch6 = Schedule(
        project_number="PRJ-2025-080",
        sample_name="IEC60068 濕熱循環測試",
        applicant_name="林工",
        device_id="CH-03",
        standard="IEC 60068",
    ...
    ex1 = SopExecution(
        ...
        device_id="CH-01",
        operator="陳工",
        test_started_at=_dt(hours=2),
    )
    ex2 = SopExecution(
        sop_id="iec60068_nb_-25_+70_3cycle",
        device_id="CH-02",
        operator="林工",
        test_started_at=_dt(hours=4, minutes=26),
    )
    ex3 = SopExecution(
        sop_id="en50155_2017_ot3_high",
        device_id="CH-04",
        operator="陳工",
        test_started_at=_dt(days=7),
        test_ended_at=_dt(days=6),
    )
    ...
    def _dwell(temp: float, n: int, jitter: float = 0.3) -> list:
    ...
    ch02_temps = (
        _linspace(25.0, -25.0, 13)
        + _linspace(-25.0, 70.0, 24)
        + _dwell(70.0, 60)
        + _linspace(70.0, -25.0, 24)
        + _dwell(-25.0, 12)
    )
    ...
    db.add_all(
        [
            ...
            DeviceCalibration(
                ...
                interval_days=365,
                certificate_number="CAL-2025-002",
                result="pass",
                created_by="林工",
                notes="低溫精度 ±0.4°C，符合規範",
            ),
            DeviceCalibration(
                device_id="CH-03",
                ...
                interval_days=365,
                certificate_number="CAL-2025-003",
                result="pass",
                created_by="陳工",
    ...
    db.add_all(
        [
            # ── f1 (M.2 2280)：2 筆 active，點擊展開見兩筆 ──────────────
            FixtureLoan(
                fixture_id=f1.id,
                borrower_name="陳工",
                device_id="CH-01",
                project_name="PRJ-2025-087",
                quantity=1,
                loan_date=_dt(hours=2),
                due_date=_dt(hours=-10),
                status="loaned",
                schedule_id=sch1.id,
            ),
            FixtureLoan(
                ...
                quantity=1,
                loan_date=_dt(days=3),
                due_date=_dt(days=-7),
                status="loaned",
            ),
            # ── f2 (PCIe x4)：1 筆 active ───────────────────────────────
            FixtureLoan(
                fixture_id=f2.id,
                borrower_name="林工",
                device_id="CH-02",
                project_name="PRJ-2025-091",
                quantity=1,
                loan_date=_dt(hours=3),
                due_date=_dt(hours=-8),
                status="loaned",
                schedule_id=sch2.id,
            ),
            # ── f3 (USB-A)：2 筆 active，其中 1 筆逾期 ──────────────────
            FixtureLoan(
                fixture_id=f3.id,
                borrower_name="張工",
                project_name="PRJ-2026-003",
                quantity=2,
                loan_date=_dt(days=10),
                due_date=_dt(days=-14),
                status="loaned",
            ),
            FixtureLoan(  # ← 逾期 5 天
                fixture_id=f3.id,
                borrower_name="黃工",
                device_id="CH-03",
                project_name="PRJ-2026-004",
                quantity=1,
                loan_date=_dt(days=20),
                due_date=_dt(days=5),
                status="loaned",
            ),
            # ── f4 (USB-C)：1 筆逾期 ────────────────────────────────────
            ...
            FixtureLoan(
                ...
                borrower_name="陳工",
                device_id="CH-04",
                project_name="PRJ-2025-072",
                quantity=2,
                loan_date=_dt(days=7),
                due_date=_dt(days=6),
                return_date=_dt(days=6),
                status="returned",
            ...
            FixtureLoan(
                ...
                borrower_name="李工",
                device_id="CH-04",
                project_name="PRJ-2025-099",
                quantity=1,
                loan_date=_dt(days=60),
                due_date=_dt(days=30),
                return_date=_dt(days=2),
                status="damaged",
                keeper_note="接頭折損，無法使用，已通報汰除",
            ),
            # ── 遺失紀錄（記錄 tab → 損壞／遺失 sub-tab）────────────────
            FixtureLoan(
                fixture_id=f4.id,
                borrower_name="吳工",
                device_id="CH-05",
                project_name="PRJ-2025-088",
                quantity=1,
                loan_date=_dt(days=45),
                due_date=_dt(days=15),
                return_date=_dt(days=5),
                status="lost",
                keeper_note="季度盤點後確認遺失，已開具遺失報告",
            ),
        ]
    )
    ...
    db.add_all(
        [
            ...
            ErrorLog(
                device_id="CH-05",
                error_type="humidity_out_of_range",
                temperature=25.1,
                humidity=32.4,
    ...
    db.add_all(
        [
            PurchaseOrder(
                fixture_id=f4.id,
                quantity=3,
                vendor="Samtec Taiwan",
                unit_price=2200.0,
                total_price=6600.0,
                status="pending",
                note="補庫存，USB-C Gen2 治具因損耗造成缺貨",
            ),
            PurchaseOrder(
                fixture_id=f3.id,
                quantity=2,
