...
- **名稱要去查資料、不是查對照表的，同樣不准把代碼丟出去**：測試條件的名稱來自法規資料
  （開放、會增減，沒有 enum 可列），查不到時一律走 `utils/conditionName.js` 寫成
  「未知條件（原碼）」。這種函式對任何輸入都有答案，不存在「漏收一項」，所以不需要
  上面那種前後端同步測試，用單元測試釘住每個分支就夠。**注意後端查不到時回的是 sop_id
  本身**（`schedule_service._get_condition_names`），所以「名稱剛好等於代碼」要當成沒查到
- **API 只回代碼、不回名稱時，由畫面自己對出名稱**：條件銜接那支（`POST /confirm-condition`）
  只回 sop_id，但按下去的畫面手上本來就有整筆排程，用 `conditionNameFromSchedule` 對位置。
  成功的 toast 是使用者唯一會看到的說明，寫代碼等於沒說
...
- 月盤點**不得靜默排除品項**（`StocktakeModal.jsx`）：有借出或預約在外的品項現場數不到完整數量、不能盤，但要列成「未納入」並寫明原因，且「涵蓋數 + 未納入數 = 治具總數」；完全沒有可盤項目時停用「完成盤點」，不能讓人按下去得到「正常 0、差異 0」的假成功。「能不能盤」一律用 `utils/fixtureStatus.js` 的 `isStocktakeCountable`，治具列的快速盤點與盤點批次的新增選單也用它；**不要拿徽章那個字判斷**（`fixtureBadgeStatus` 只挑一個字，「即將不足」會蓋掉借出）
- 排程詳情的「▶ 立即開始」與「開始第 N 條件」，在設備非待機、正落在維護時段，或同台還有另一筆排程沒結案時**一律停用並就地寫出原因**（`ScheduleDetailModal.jsx` 的 `describeDeviceReadiness`）：畫面手上已經有設備現況，還讓人按一次注定被後端拒絕的操作，只會讓人以為是排程壞了。**後端檢查不得因此移除**，那層擋的是競態

### 「維護中」和「身上有排程」是兩件事

設備清單送兩組欄位，不要把它們合起來看：**`maintenance_blocked` / `maintenance_reason` /
`maintenance_end_at` 是維護時段**，會擋住啟動；**`running_schedule_note` 是這台身上還有沒結案的
排程**，只是一句說明，不代表設備不可用。那筆沒結案時，後端的手動啟動與排程啟動都會等它結案
（`schedule_service.running_schedule_for_device`），但那是「要等別筆」，不是「設備壞了」。

- **要不要標成「不可用」，一律只看維護**。目前的呼叫點是設備卡的紅色徽章與 🔒 原因、
  SOP 面板的鎖定訊息與測試選擇區、頂部的「不可用」計數、排程詳情的指派下拉與兩顆開始鈕。
  兩顆開始鈕另外還會因為同台有另一筆進行中排程而停用，判斷用排程清單，不讀 `running_schedule_note`
- **`running_schedule_note` 只能拿來顯示**，用中性樣式，不給鎖頭。SOP 面板是唯一的例外：那裡要
  把測試選擇整段收起來並寫出「請到排程頁面操作」，因為後端手動啟動會回 409（`sop.py`），
  留著等於邀請使用者走一條一定被擋回來的路
- **計數的「不可用」要兩個條件都成立：待機中，而且排了維護**，跟設備卡的紅色徽章同一條判準。
  只數旗標的話，正在跑又被排了維護的機器會同時進「執行中」和「不可用」，四個數字加起來比設備
  總數還多，而畫面上只看得到幾個各自合理的數字（`tests/e2e/specs/top-bar-counts.spec.js` 用
  「加總不超過設備總數」守著）
- **推導只做一次**：「有排程掛著、而且現在待機」這條判斷收在 `constants.js` 的 `deviceScheduleNote`，
  設備卡與 SOP 頁共用，`SOPPage` 再把結果傳給 `ControlPanel`；`ControlCenter` 只送設備真正的狀態，
  維護另外走 `liveDeviceMaintenance`。以前四個地方各自從一個混合旗標推導，同一台機器在設備卡、
  頂部計數、排程詳情上半與指派下拉得到四種說法

### 測試何時結束、跑到第幾圈，讀後端的答案

「這台設備幾點空出來」由後端算好放在設備清單的 `estimated_end_at`（後端那份的組成見
`state-machine.md`），**前端不得拿測試條件自己重算**。倒數一律用 `useCountdown.js`，
設備卡與 SOP 左側資訊面板共用它。

- **自己算會少兩段**：測試跑完要回常溫再穩定 30 分鐘，那段設備仍然占用；暫停的時間也要加回去。
  SOP 面板以前用 `generateSP` 的曲線長度自己算，於是暫停時設備卡的倒數停住、面板繼續減，
  穩定期間面板已經顯示 0、設備卡還剩半小時——同一台機器兩個畫面講出不同的結束時間
- **`generateSP` 只負責畫溫度曲線**，不要再拿它的長度當時間來源
- **`sim_cycle` 是「已經跑完幾圈」，不要加一當成「正在跑第幾圈」**：後端在最後一圈結束時就把它
  加到總圈數了，加一會讓三循環的測試跑完顯示 0004/0003，而且一路掛到降溫與常溫穩定結束。
  講法跟同一塊面板的 Step（做完幾步／共幾步）一致。同頁的自動步驟觸發判斷讀的是原始值、
  沒有加一，別讓同一份資料在同一頁有兩種意思
- 這幾格的格式化收在 `utils/executionInfo.js`（`formatFreeTime` / `formatCycle` / `formatStep`），
  寫在 JSX 裡的話錯了不會有東西紅；`client/src/__tests__/executionInfo.test.js` 釘著那個加一

### 排程跑到第幾條件，只有一份解讀

`utils/scheduleProgress.js` 是唯一的來源，設備卡、頂部橫幅、SOP 頁的等待確認卡、排程詳情與
甘特圖都吃它，**不要在第六個地方自己讀 `current_condition_index`**。

- **設備停下來等人確認時，畫面不得宣稱誰完成了**。後端那個索引在測試自然跑完時加 1、被人按停時
  原地不動，排程資料分不出是哪一種，所以那個當下只說下一步能做什麼（`conditionProgress` 的
  `actionLabel` / `nextConditionName`）。以前各畫面自己加 1 再顯示，單條件測試跑完會寫成
  「第 2/1 條件」、三條件寫成「(4/3)」，被按停的還會寫「條件 1/1 已完成」——那時一條都沒完成
- **「這台是不是停著等人確認」用 `isWaitingForConfirm`，一定要連設備狀態一起看**。只看「有沒有
  排程掛著」的話，緊急停止中的機器也會冒出「開始第 N 條件」，而那個畫面正是不該有任何開始測試
  入口的時候（`tests/e2e/specs/emergency-screen.spec.js` 守著）
- 後端 `running_schedule_note` 同理不報進度（只在設備待機時顯示，那時沒有哪一條正在跑），
  進度與下一步一律由前端這份資料講
