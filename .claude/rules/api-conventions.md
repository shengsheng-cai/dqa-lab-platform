...
- **治具保管人的唯一寫入口是 `PATCH /api/fixtures/{id}/keeper`**（指定人員）。`FixtureUpsert`（新增／編輯治具）**不收 `keeper_name`**，而且 `extra="forbid"`：多送直接 422，不收下再忽略——忽略等於「回你成功、其實什麼都沒做」，那正是這個欄位原本的病（兩個寫入口、顯示優先用人員那邊的名字，於是在編輯治具改保管人會存進 DB 卻不改變畫面）。`Fixture.keeper_name` 降為顯示快照，只由 `set_keeper` 與 Excel 匯入寫入；人員被刪除時外鍵 `SET NULL`，留下這個名字當「未連結人員」的線索
- **Excel 匯入是唯一還會產生「未連結文字保管人」的入口**：名字剛好對到一位在職人員才連（顯示名稱沒有唯一限制，同名或查無此人一律留文字、不猜是哪一個）。匯出要寫那個人**現在**的顯示名稱而非快照——匯出的檔案會被改過再匯回來，那時是拿名字對人，寫舊名字會讓改過名的人對不回自己
- **匯入只覆蓋表格裡真的有的欄位**（含現有數量）：欄位不在表格上就不要動它。以前是整份覆蓋，拿一份只有數量的表格來更新庫存，會順手把備註、廠商、保管人清空，保管人那一項還等於繞過畫面上的清除確認
