...
| **R-12** | An irreversible operator action fires on a single click, destroying work that is already in flight | Medium | Medium | P1 | `tests/e2e/specs/device-stop-confirm.spec.js`, `tests/e2e/specs/fixture-loan.spec.js`, `tests/e2e/specs/maintenance-block.spec.js`, `tests/e2e/specs/purchase-arrival-confirm.spec.js`, `tests/e2e/specs/users-page-feedback.spec.js` | Only the most damaging actions are guarded so far. Deleting a purchase order or schedule, and the inline quick-stocktake field all still act on one click, or confirm without naming what they are about to act on; those remain in the backlog |
