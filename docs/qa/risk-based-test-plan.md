...
| **R-04** | UI shows stale or misleading state, or a successful write does not persist the value the operator submitted | Medium | High | P1 | `schedule-flow.spec.js`, `fixture-loan.spec.js`, `test_maintenance.py`, `stocktake-scope.spec.js` | A transient refresh/network failure can still require manual retry; optional-field semantics need an explicit regression whenever a new nullable field is added |
...
| **R-12** | An irreversible operator action fires on a single click, destroying work that is already in flight | Medium | Medium | P1 | `tests/e2e/specs/device-stop-confirm.spec.js`, `tests/e2e/specs/fixture-loan.spec.js`, `tests/e2e/specs/maintenance-block.spec.js`, `tests/e2e/specs/purchase-arrival-confirm.spec.js` | Only the most damaging actions are guarded so far. Deleting a purchase order or schedule, and the inline quick-stocktake field all still act on one click, or confirm without naming what they are about to act on; those remain in the backlog |
...
2. Fixture reserve → loan → return lifecycle, including invalid quantities and
   repeated actions.
3. Browser schedule confirmation, visible status reconciliation, and maintenance partial-update semantics.
