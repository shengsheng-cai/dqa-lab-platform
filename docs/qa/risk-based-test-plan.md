...
| **R-07** | An external service or report fails a core operation, or a report is produced but silently incomplete | Medium | Medium | P2 | `test_line_resilience.py`, `test_reports_degradation.py` (degradation plus the timestamp contract the measurement summary depends on), `test_ai_observability.py` | Live provider behavior and quota changes are excluded; a frontend save path that omits a field is caught by review, not by a test (GAP-04) |
