...
| **REQ-FIX-02** | Dates the operator submits — actual return date and default due date — are recorded as the operator's local calendar day, not the UTC one | R-06 | `client/src/__tests__/timezone.test.js` (suite pinned to `Asia/Taipei`) | [BUG-004](BUG-004-fixture-dates-stored-one-day-early.md) | Covered at helper level |
