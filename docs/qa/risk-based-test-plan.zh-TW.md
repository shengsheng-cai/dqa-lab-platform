...
| **R-07** | 外部服務或報告讓核心操作失敗，或報告產出了但內容悄悄不完整 | Medium | Medium | P2 | `test_line_resilience.py`、`test_reports_degradation.py`（降級，以及量測摘要所依賴的時間契約）、`test_ai_observability.py` | 供應商的實際行為與額度變動不涵蓋；前端存檔路徑漏送欄位只能靠審查抓，沒有測試守（GAP-04） |
