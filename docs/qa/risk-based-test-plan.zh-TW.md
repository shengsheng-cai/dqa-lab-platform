...
| **R-14** | 防暴力破解不是被程式碼、而是被部署設定弄失效：鎖定以來源位址計數，而一個伺服器並不信任的平台轉送層會把所有訪客收斂成同一個計數器，於是任何來源失敗五次，就有十分鐘拒絕所有人——包括手上已經有有效憑證的 session | Medium | Medium | P1 | `test_deploy_config.py`、`test_login_rate_limit.py` | 測試釘住計數行為，也檢查啟動指令裡那個信任值實際上會信任誰、而不是它怎麼拼；平台的轉送層有沒有一直待在那個網段之內，測試看不到，它一搬家就會安靜地回到全站共用一個計數器 |
