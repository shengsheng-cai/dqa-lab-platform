...
| **R-04** | UI 顯示過期或誤導狀態，或成功寫入後沒有保存操作人員送出的值 | Medium | High | P1 | `schedule-flow.spec.js`、`fixture-loan.spec.js`、`stocktake-scope.spec.js`、`schedule-start-readiness.spec.js`、`fixture-keeper.spec.js`、`modal-layout.spec.js`、`login-required-fields.spec.js`、`list-load-errors.spec.js`、`maintenance-date-fields.spec.js`、`test_maintenance.py`、`client/src/__tests__/setpoints.test.js` | 短暫的重新整理或網路失敗仍可能需要人工重試；新增可空欄位時仍要明確補上更新語意的回歸測試 |
...
| **R-07** | 外部服務或報告讓核心操作失敗，或報告產出了但內容悄悄不完整、前後矛盾，或識別錯對象 | Medium | Medium | P2 | `test_line_resilience.py`（含推播的收件人與內容）、`test_line_webhook_signature.py`（對內 webhook 的 HMAC 簽章）、`test_reports_degradation.py`（降級、量測摘要所依賴的時間契約、數據統計與不確定度分析的一致性契約、受測樣品識別契約、雙溫循環的分段統計，以及 CSV 與 PDF 的欄位同源）、`test_uncertainty.py`、`test_ai_observability.py`、`client/src/__tests__/executionPayload.test.js` | 供應商的實際行為與額度變動不涵蓋；前端存檔路徑漏送欄位現在會讓測試變紅，但備援路徑何時觸發仍然只能靠審查抓（GAP-04） |
...
| **R-14** | 防暴力破解不是被程式碼、而是被部署設定弄失效：鎖定以來源位址計數，而一個伺服器並不信任的平台轉送層會把所有訪客收斂成同一個計數器，於是任何來源失敗五次，就有十分鐘拒絕所有人——包括手上已經有有效憑證的 session | Medium | Medium | P1 | `test_deploy_config.py`、`test_login_rate_limit.py` | 測試釘住計數行為，也檢查啟動指令裡那個信任值實際上會信任誰、而不是它怎麼拼；平台的轉送層有沒有一直待在那個網段之內，測試看不到，它一搬家就會安靜地回到全站共用一個計數器。它確實在那個網段內、而且是把來源接在轉送標頭後面而非把呼叫端的原樣傳過來，已於 2026-09-08 對部署環境實際探測確認；平台有變動時值得重打一次 |
...
3. 瀏覽器上的排程確認、可見狀態與設備可啟動狀態的一致性、治具保管人一致性，
   以及維護紀錄部分更新語意。
4. 登入失敗鎖定：封鎖本身、封鎖期間連正確密碼也拒絕、成功後計數歸零，
   以及計數 key 所依賴的那個部署設定。
...
- 被選為正式案例的缺陷能從[追溯表](traceability.zh-TW.md)連過去；其他修正由 commit 與回歸測試保留證據。
