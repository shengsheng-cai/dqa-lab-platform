...
* **✅ SOP 執行頁 (SOPPage)**:
    ...
    * **完整波型曲線**: SP 目標曲線（灰虛線）+ PV 實際曲線（紅實線）疊加，X 軸為完整測試時長；循環測試幾次畫幾個波；Brush 縮放。
...
| 完整波型曲線 | ✅ | SP 虛線 + PV 實線疊加，X 軸完整測試時長，循環幾次畫幾波 |
...
| 捲動架構修復 | ✅ | index.css / Dashboard.jsx / SOPPage.css 三層修正 |
